...
import com.tpa.videogames.repositories.RepositorioEstudio;
...
public class ServicioEstudio implements ServicioBase<Estudio> {
    ...
    @Transactional
    public List<Estudio> findByName(String q) throws Exception{
        try{
            List<Estudio> entities = this.repositorio.findByName(q);
            return entities;
        } catch (Exception e) {
            throw new Exception(e.getMessage());
        }
